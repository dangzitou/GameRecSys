...
public class RecSysServer {
    ...
    public void run() throws Exception{
        ...
        if (webRootPath == null) {
             throw new IllegalStateException("Unable to determine webroot URL location. Checked classpath and src/main/resources/webroot/");
        }

        System.out.println("Resolved Data Path: " + webRootPath);

        // Load ONLY Game Data. MovieLens data (links, ratings, embeddings) is incompatible with Game AppIDs.
        // Passing null/empty for other paths.
        DataManager.getInstance().loadData(webRootPath + "sampledata/games_filtered.csv",
                "", "", "", "", "i2vEmb", "uEmb");

        // Verify data loaded
        List<com.sparrowrecsys.online.datamanager.Movie> check = DataManager.getInstance().getMovies(10, "positiveReviews");
